...
An invalid or unreadable state never mutes audio: the line reports its error in the log and the analysis panel, and the rest of the configuration keeps running. The Editor edits the same state as a card; the standalone `EAPO XT Bass Management` VST3 plugin runs the identical DSP core and exchanges the same JSON via its plugin state, so presets move between the two unchanged.
