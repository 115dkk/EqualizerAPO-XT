...
* **External libraries** — **not** committed to the repository: [FFTW](https://www.fftw.org/), [libsndfile](https://libsndfile.github.io/libsndfile/), [muParserX](https://github.com/beltoforion/muparserx), [TCLAP](http://tclap.sourceforge.net/), [Google Highway](https://github.com/google/highway), and the MIT-licensed Steinberg VST3 *pluginterfaces*. They are placed under `deps/` at build time.
...
* **Tests** — four executable test suites (`HybridConvTests`, `EditorLogicTests`, `EngineOrchestrationTests` and `AudioRegressionTests`) plus `TestVst2Plugin` and `TestVst3Plugin` host fixtures.
...
CI lives in `.github/workflows/build.yml`: it builds the x64 `sse2`, `avx`, `avx2`, `avx512` and `avx10_1` variants plus ARM64 `neon`, then calls `vpk pack` to produce the per-channel installers (see [the installation section](Documentation#installation) and [SIMD build matrix](https://github.com/115dkk/EqualizerAPO-XT/blob/main/docs/SimdBuildMatrix.md)). Pull requests build AVX2 only. A push to `main` builds all six and creates a release only when the automatic version step produces a new version; docs-, CI- and refactor-only pushes skip the build matrix. Manual workflow runs build all six. `publish-wiki.yml` separately publishes changes under `Wiki/github-wiki/` to this wiki. The original NSIS installer has been removed.
