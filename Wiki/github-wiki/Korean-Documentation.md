...
* **WASAPI 독점 모드**(재생기의 'WASAPI 독점' 출력, 스트리밍 앱의 '독점 모드' 스위치, HQPlayer, JRiver 등)는 소리를 응용 프로그램에서 드라이버로 바로 보냅니다. 엔진이 보지 못하므로 APO는 실행되지 않고, 장치 선택기의 어떤 옵션도 이것을 바꾸지 못합니다. 방법은 셋입니다. 응용 프로그램의 출력을 공유 모드로 바꾸거나, 응용 프로그램에 ASIO 출력이 있으면 `<드라이버 이름> (EQ APO XT)` 항목을 고르거나([ASIO](https://github.com/115dkk/EqualizerAPO-XT/blob/main/docs/features/asio.md)), Windows 소리 설정에서 장치 속성의 **고급** 탭을 열어 **응용 프로그램에서 이 장치를 단독으로 제어할 수 있도록 허용**을 해제해 독점 모드 자체를 막는 것입니다. 해제하면 독점 모드를 요청한 응용 프로그램은 공유 모드로 내려오거나, 출력 설정을 바꿀 때까지 오류를 냅니다.
