...
지원하는 명령 이름이 없는 줄은 아무 동작도 하지 않는 텍스트입니다. 주석이 이렇게 동작합니다. `#`로 시작하는 줄은 인식되는 명령이 아니며, 존재하지 않는 명령 이름도 무시됩니다. 명령 이름은 대소문자를 구분합니다. `Preamp`는 동작하지만 `preamp`는 동작하지 않습니다. `Copy`, 두 컨볼루션 명령, `VSTPlugin`, `Hilbert`, `Velvet`, `SubwooferRouting`, `LoudnessCorrection`처럼 문법을 엄격히 검사하는 명령은 잘못된 입력의 파일·줄·이유를 `EqualizerAPO.log`와 Editor의 분석/카드 오류에 남깁니다. 뒤의 줄은 계속 불러옵니다. 일부 예전 명령 파서는 자체 로그 메시지만 남깁니다.
...
64비트 VST2(`.dll`) 또는 VST3(`.vst3`) 플러그인을 불러와 현재 선택된 채널을 처리합니다. 상대 라이브러리 경로는 EqualizerAPO-XT의 `VSTPlugins` 폴더를 기준으로 해석하며, 절대 경로로 다른 위치를 가리킬 수도 있습니다. Editor에서 모듈을 고르고 패널을 열거나 카드에 삽입하며 상태를 저장할 수 있습니다. 상태는 따옴표로 감싼 `ChunkData` 또는 숫자 파라미터 키/값 쌍 중 한 형태로 기록하며, 정규화된 한 줄에 둘을 함께 쓰지 않습니다.
...
내장 프리셋 `Issue #246 - Front/Rear 4.1`은 이슈 #246의 전방/후방 4.1 구성을 재현합니다. 전방 메인은 80 Hz 하이패스와 4차 80 Hz 전방 베이스 경로, 후방 메인은 100 Hz와 60 Hz 후방 베이스 경로, LFE 입력은 +10 dB로 보존, 합산 행렬에는 -14 dB LFE 피드가 들어갑니다. 출력은 원본 저수준 필터 사슬과 샘플 단위로 일치합니다.

잘못되거나 읽을 수 없는 상태가 소리를 끊는 일은 없습니다. 해당 줄이 로그와 분석 패널에 오류를 남기고, 나머지 설정은 계속 실행됩니다. Editor는 같은 상태를 카드로 편집하고, 독립 배포되는 `EAPO XT Subwoofer Routing` VST3 플러그인은 동일한 DSP 코어를 돌리며 플러그인 상태로 같은 JSON을 주고받으므로, 프리셋이 양쪽을 그대로 오갑니다.
