...
### BassManagement
**문법:** `BassManagement: State <압축 JSON 상태>`
**문법:** `BassManagement: Profile <프로필 파일 (*.bmxt.json)>`

베이스 매니지먼트 전체를 한 줄로 실행합니다. 스피커 그룹별 크로스오버, 전용 베이스 경로, 물리 LFE 입력을 별도 소스 경로로 보존하는 처리, 경로별 게인·극성·지연·EQ, 그리고 처리된 경로를 물리 채널로 되섞는 출력 행렬이 들어 있습니다. 이런 구성을 만들 때 필요하던 `Copy` → `Channel` → `Filter` → `Copy` 사슬을 대신합니다. 이 명령은 현재 `Channel:` 선택과 무관하게 모든 채널을 보며, 상태가 참조하지 않는 채널은 그대로 통과합니다.

상태는 스키마 `equalizerapo.xt.bass-management`를 갖는 JSON 문서입니다. `State`는 그 JSON을 줄 안에 담고, `Profile`은 같은 JSON을 UTF-8 파일에서 읽습니다. 경로는 설정 파일 기준의 상대 경로이고, 따옴표를 쓸 수 있으며, `%USERPROFILE%` 같은 환경 변수를 허용합니다. 상태는 물리 입력을 이름 있는 main/bass/source-LFE 경로로 섞고(source-LFE 경로는 출력 행렬이 나중에 LFE 출력을 덮어써도 원본 LFE 입력을 읽습니다), 경로마다 처리한 뒤 출력에 합산합니다. 헤드룸은 자동(컴파일된 응답을 20 Hz부터 나이퀴스트까지 분석해 예상 피크가 0 dBFS를 넘지 않게 공통 트림을 정함) 또는 dB 단위 수동 트림입니다.

```
BassManagement: Profile "BassManagement\Issue 246 Front Rear 4.1.bmxt.json"
```
...
잘못되거나 읽을 수 없는 상태가 소리를 끊는 일은 없습니다. 해당 줄이 로그와 분석 패널에 오류를 남기고, 나머지 설정은 계속 실행됩니다. Editor는 같은 상태를 카드로 편집하고, 독립 배포되는 `EAPO XT Bass Management` VST3 플러그인은 동일한 DSP 코어를 돌리며 플러그인 상태로 같은 JSON을 주고받으므로, 프리셋이 양쪽을 그대로 오갑니다.
