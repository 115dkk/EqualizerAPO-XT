...
### MultiConvolution
**문법:** `MultiConvolution: <출력 채널> <다채널 임펄스 응답>`

BRIR(바이노럴 룸 임펄스 응답)이나 크로스피드 설정처럼, `Convolution`의 제자리 1:1 매핑만으로는 부족한 경우를 위한 컨볼버입니다. 앞선 `Channel:` 명령으로 선택한 채널을 모두 읽어, 선택된 각 입력을 지정한 다채널 임펄스 응답 파일의 대응 채널과 합성곱하고, 그 결과를 모두 더해 명령 줄의 첫 토큰으로 지정한 출력 채널 하나에 씁니다(선택된 입력 *i*는 임펄스 응답 채널 *i* mod 파일의 채널 수와 짝지어지므로, 모노 파일이면 모든 입력에 같은 채널이 적용됩니다). 경로와 샘플레이트 규칙은 `Convolution`과 같습니다. 파일 이름은 설정 파일 기준 상대 경로이고, 큰따옴표로 감쌀 수 있으며, `%USERPROFILE%` 같은 환경 변수를 넣을 수 있고, libsndfile이 지원하는 형식이어야 하며, 샘플레이트는 장치와 같아야 합니다. 경로가 잘못되었거나 샘플레이트가 맞지 않거나 파일에 쓸 수 있는 오디오가 없어도 출력 채널 자체는 만들어지고 무음만 나오므로, 이후 채널 선택이 밀리지 않습니다. 양쪽 귀를 모두 처리하는 바이노럴 구성이라면 입력이 덮어써지기 전에 `Copy:`로 복제하고, 각 귀마다 `MultiConvolution`을 하나씩 둬야 합니다.

```
# 왼쪽 귀용 2채널 BRIR을 채널 L에 합산
Channel: L R
