...
### MultiConvolution
**문법:** `MultiConvolution: <채널>=<파일 채널>[+<파일 채널>...] [<채널>=... ...] <다채널 임펄스 응답>`
...
매핑 대상으로 아직 없는 채널 이름을 쓰면 가상 채널로 만들어집니다(앞선 줄이 그 채널에 쓴 적이 없으면 무음으로 처리됩니다). 파일 채널 수를 넘는 번호는 무음만 더하고 로그에 남습니다. 경로와 샘플레이트 규칙은 `Convolution`과 같습니다. 파일 이름은 설정 파일 기준 상대 경로이고, 큰따옴표로 감쌀 수 있으며, `%USERPROFILE%` 같은 환경 변수를 넣을 수 있고, libsndfile이 지원하는 형식이어야 하며, 샘플레이트는 장치와 같아야 합니다. 경로가 잘못되었거나 샘플레이트가 맞지 않거나 파일에 쓸 수 있는 오디오가 없어도 매핑된 채널 자체는 만들어지고 무음만 나오므로, 이후 채널 선택이 밀리지 않습니다. Editor에서는 카드가 파일과 함께 채널 매핑 화면을 스킨마다 다른 모습으로 보여줍니다. 입력은 파일의 채널이고, 출력은 설정의 채널과 직접 추가한 가상 채널입니다.
