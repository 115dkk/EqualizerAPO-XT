...
- ASIO 출력이 없는 독점 모드 응용 프로그램(스트리밍 서비스 데스크톱 앱의 '독점 모드')은 Windows의 장치 속성에서 독점 제어를 끄는 것으로 공유 모드에 돌려세울 수 있습니다. 이것은 문제 해결 문서에 넣었고, 장치 선택기의 옵션으로 만들 수도 있습니다(6절).
...
| 문제 해결 | 위키 문제 해결 절에 네 종류의 스트림을 적었다. 장치 속성의 '독점 제어 허용' 해제가 즉시 쓸 수 있는 방법이다 |
...
| 판정 | 문제 해결 절에 적음. 선택기 옵션은 선택 | **권장** | 2026-08-29에 폐기(연구 문서 12절). 재제안하지 않음 | 제외 |
...
## 6. E1: 독점 제어 차단을 문제 해결 옵션으로

Windows 소리 설정의 장치 속성(고급 탭)에 있는 "응용 프로그램에서 이 장치를 단독으로 제어할 수 있도록 허용"이 이것입니다. 끄면 독점 요청이 `AUDCLNT_E_EXCLUSIVE_MODE_NOT_ALLOWED`로 거부되고, 응용 프로그램은 공유 모드로 내려오거나(대부분의 재생기) 출력 설정을 바꾸라는 오류를 냅니다. 그 뒤의 스트림은 엔진을 지나므로 EQ가 붙습니다. 이 방법은 문서만으로 끝나며, 위키 문제 해결 절에 적었습니다.

장치 선택기에 같은 것을 체크박스로 두는 것은 선택입니다. 값은 엔드포인트 `Properties` 키의 `{b3f8fa53-0004-438e-9003-51a46e139bfc},3`(허용, DWORD, 없으면 허용)과 `,4`(독점 앱 우선)이며, 커뮤니티가 소리 설정의 동작에서 확인한 값입니다. 메인테이너 기기의 `Properties`와 `FxProperties`는 같은 ACL(Administrators SetValue)이라 장치 선택기의 레지스트리 포트가 지금 FxProperties에 쓰는 권한으로 쓸 수 있습니다. 소리 설정 자체는 문서화되지 않은 `IPolicyConfig::SetShareMode`를 쓰므로, 레지스트리에 쓴 값이 실행 중인 AudioSrv에 언제 반영되는지(서비스 재시작이 필요한지)는 게이트로 확인해야 합니다. 판정은 `CaptureProbe --exclusive`(추가할 옵션)가 값을 끈 뒤 `AUDCLNT_E_EXCLUSIVE_MODE_NOT_ALLOWED`를 받는지로 합니다.
...
- 레지스트리로 끈 독점 허용 값이 서비스 재시작 없이 반영되는지(6절).
- Windows 11에서 raw 모드의 MFX 로드 여부. Microsoft 문서는 Windows 10까지만 적습니다. 캡처 게이트의 raw 라운드는 VB-CABLE이 raw를 지원하지 않아 잴 수 없었습니다.

**메인테이너 결정**

1. E2를 진행할지, 진행한다면 첫 릴리스 범위를 재생만으로 할지 캡처까지 할지.
2. 장치 선택기에서 WASAPI 대상 항목을 엔드포인트 행의 옵션으로 둘지(권장), 별도 행으로 둘지.
3. E1 체크박스를 장치 선택기에 넣을지, 문서로만 둘지.
4. 3절의 저지연 검증 라운드를 캡처 게이트에 넣을지. 넣는다면 독립 PR로 먼저 할 수 있습니다(E2와 무관).

---

## 8. 구현 순서

E2를 진행한다면 순서는 다음이 자연스럽습니다. 각 단계는 앞 단계 없이도 CI에서 초록이 됩니다.

1. **저지연 검증 라운드**(3절): `CaptureProbe --period min`, 게이트의 `CABLE Input` 재생 라운드, 컨볼루션 판정. 결과에 따라 컨볼루션 파티션 분리 여부를 정합니다.
2. **`WasapiExclusiveTarget`**: `asio/WasapiExclusiveTarget.{h,cpp}`, `IAudioClient` 시임의 가짜 클라이언트 단위 테스트, `AsioProbe --target wasapi:{guid}`.
