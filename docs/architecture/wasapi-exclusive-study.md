...
## 10. 재검토: 독점 모드 앱이 고르는 엔드포인트에 EQ를 둘 수 있는가 (2026-09-24)

감사 #348을 처분하면서 메인테이너가 `--exclusive-mode-eq` 옵션 이름을 정리하기 전에 원래 구상을 다시 보자고 했습니다. 지금 이 옵션은 엔드포인트를 ASIO 드라이버 목록의 항목('<장치> - <엔드포인트> (EQ APO XT)')으로 만들어 줍니다. ASIO 전용 앱에는 쓸모가 있지만, 원래 원한 것은 독점 모드로 재생하는 앱이 Windows 소리 장치 목록에서 고르는 엔드포인트 자체에 EQ가 걸리는 것이었습니다. 4절에서 '재제안하지 않음'으로 닫은 E3를 메인테이너의 요청으로 다시 엽니다.

### 10.1 전제는 그대로다

독점 모드 스트림은 오디오 엔진을 거치지 않고 드라이버의 버퍼로 바로 갑니다. APO는 엔진 그래프 안의 객체라 끼어들 자리가 없습니다. Windows 11의 새 APO 인터페이스(`IAudioSystemEffects3` 등)도 엔진 그래프 안에서만 동작합니다. 따라서 '독점 모드 앱이 고르는 엔드포인트에서 EQ가 걸린다'는 것은 그 엔드포인트를 우리가 제공하거나, 누군가 제공한 엔드포인트 뒤에 우리 처리를 두는 것 말고는 방법이 없습니다.

### 10.2 가능한 구조 둘

| 기준 | V1. 자체 가상 엔드포인트 드라이버 | V2. 서명된 가상 케이블 + 브리지 |
|---|---|---|
| 앱에 보이는 것 | 'EQ APO XT - <장치>' 같은 우리 엔드포인트 | 사용자가 설치한 케이블의 재생 쪽(이름은 소리 설정에서 바꿀 수 있음) |
| 새로 만드는 것 | 커널 WaveRT 가상 오디오 드라이버(SysVAD 예제나 Scream 계열에서 출발) + 호스트와 오디오를 주고받는 통로 | 호스트 안의 브리지: 케이블의 녹음 쪽을 읽어 엔진에 넣고, 실제 장치를 독점으로 엶(출력 절반은 지금의 `WasapiExclusiveTarget`) |
| 서명 | Windows 10/11 x64 커널 드라이버라 Microsoft Partner Center의 attestation 서명이 필요하고, 그 계정에는 EV 코드 서명 인증서가 필요함. 코드 서명은 메인테이너가 보류 중 | 우리 쪽 새 드라이버 없음. 케이블은 제작사가 서명함 |
| 시계 | 드라이버의 위치 보고를 실제 장치의 시계에 묶어 설계하면 시계가 하나로 유지됨 | 케이블과 실제 장치의 시계가 달라 드리프트가 쌓임. 적응형 리샘플링이나 샘플 삽입·삭제로 맞춰야 하므로 비트 퍼펙트가 아님 |
| 지연 | 가상 장치 주기 + 호스트 처리 + 실제 장치 주기 | 케이블 주기 + 호스트 처리 + 실제 장치 주기 + 드리프트 보정 버퍼 |
| 배포 | 설치 파일이 드라이버를 설치(장치 관리자·재부팅 흐름, 백신 평판 문제가 커짐) | 케이블은 사용자가 따로 설치해야 함(VB-CABLE을 설치 파일에 함께 넣을 수 있는지는 제작사 라이선스를 확인하지 못함) |
| 선례 | Voicemeeter, Realphones, SteelSeries Sonar | Voicemeeter의 '가상 입력' 사용법, 여러 DIY 루프백 설정 |

이 밖에 앱 프로세스에 들어가 `IAudioClient`를 가로채는 방법(E4)은 백신 오탐과 게임 안티치트 충돌 때문에 여전히 제외합니다.

### 10.3 판단

독점 모드 앱을 앱 쪽 설정 없이 잡는 구조는 V1과 V2뿐입니다. V1은 원래 구상에 가장 가깝고 시계를 하나로 유지할 수 있지만, 커널 드라이버 개발과 attestation 서명을 전제로 합니다. 코드 서명을 도입하지 않는 한 사용자에게 배포할 수 없으므로, V1을 고르는 것은 곧 코드 서명을 도입하는 결정입니다. V2는 지금 가진 부품(엔진 호스트, WASAPI 독점 대상)으로 지을 수 있지만, 사용자가 가상 케이블을 따로 설치해야 하고 두 시계를 맞추는 드리프트 보정이 새로 필요하며 결과는 비트 퍼펙트가 아닙니다. 독점 모드를 고르는 이유가 리샘플링 없는 원본 재생이므로 V2는 그 이유와 부딪힙니다.

ASIO 항목은 이 둘 가운데 어느 것을 골라도 ASIO 전용 앱을 위해 남깁니다. 어느 구조로 갈지는 메인테이너가 정합니다.

**결정(2026-09-24, 메인테이너):** V1과 V2 모두 하지 않습니다. 엔드포인트의 ASIO 항목을 그대로 두고, 그 항목에 없던 고급 설정을 ASIO 드라이버 항목과 맞춥니다. 장치 선택기에서 엔드포인트의 'ASIO 앱에서 사용'을 켜면 문제 해결 옵션 안에 버퍼 제거와 대기 시간, 부팅 시 엔진 호스트 자동 시작, 32비트 호스트 지원이 펼쳐지고, 끄면 다시 접힙니다. 명령줄 옵션 `--exclusive-mode-eq`는 지금 뜻에 맞는 `--asio-entry`로 바꾸고 옛 이름은 별칭으로 남깁니다.
