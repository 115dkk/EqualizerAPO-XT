...
CSS = '''
  ...
  .strip .cap { font-size: 8px; letter-spacing: 1.5px; }
...
def body(skin, missing):
    ...
    if skin == 'rack':
        if not missing:
            return f'<div class="line"><span class="rlamp"></span><div class="strip"><span class="cap">PATCH</span><span class="name" style="font-size:12.5px">example.txt</span></div>{ib("folder")}{ib("pencil")}</div>'
        return f'<div class="line"><span class="rlamp red"></span><div class="strip"><span class="cap">PATCH <span style="color:var(--accent);margin-left:6px">NOT FOUND</span></span><span class="name dim" style="font-size:12.5px">missing.txt</span></div><span class="btn lit">LOCATE</span>{ib("pencil", True)}</div>'
    if skin == 'matrix':
        if not missing:
...
page('ReferenceCard', 'Rows', 1180, 'The Include row body: a found reference with its path window, and a missing one, five skins', CSS, frames)
