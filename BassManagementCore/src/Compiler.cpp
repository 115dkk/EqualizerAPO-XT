...
void forEachFilter(
	const BassManagementState& state,
...
bool pathDependencyGraphHasCycle(const BassManagementState& state)
{
...
ValidationResult validate(const BassManagementState& state)
{
	ValidationResult result;

	validateUtf8Field(result, state.schema, std::string(), "/schema");

	if (state.schema != kBassManagementSchema)
	{
		addDiagnostic(
			result,
			DiagnosticSeverity::Error,
			...
			std::string(),
			"/schema",
			"The state schema is not the bass-management schema.");
	}

	if (state.version != kBassManagementSchemaVersion)
	{
		addDiagnostic(
			result,
			DiagnosticSeverity::Error,
