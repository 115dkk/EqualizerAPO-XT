...
- **시스템 코드 페이지 밖의 문자가 든 사용자 이름이나 다른 관리자 계정으로 승인한
  설치에서도 설정 폴더가 맞게 잡힙니다.** 설치 단계가 `%LOCALAPPDATA%`의 ANSI
  사본에서 폴더를 만들어, 코드 페이지에 없는 문자가 `?`로 바뀌고 오디오 엔진이 없는
  폴더를 읽었습니다. 다른 계정으로 승인하면 그 계정의 프로필에 폴더가
  생겼습니다. 이제 설치를 시작한 사용자의 프로필에서 폴더를 잡습니다([#349](https://github.com/115dkk/EqualizerAPO-XT/pull/349)).
