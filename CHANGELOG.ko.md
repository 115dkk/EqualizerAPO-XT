...
- 분석 패널의 응답 그래프(패널의 마지막 색놀이)가 다섯 계기가 됐습니다.
  ...
  감시하고, Precision Minimal은 플로터 기록지(1px 잉크 선, 초과 영역 빗금,
  십자선 주석)를 인쇄하며, Soft Lab은 친절한 파스텔 지형(부스트 언덕, 컷
  골짜기, 'Over 0 dB' 칩)을 보여주고, Hardware Rack은 SPECTRUM MONITOR
  스코프(0 위에서 앰버로 타는 녹색 인광 트레이스, OVER 램프)를 앉혔으며,
  Signal Matrix는 보드 텔레메트리(시안 트레이스, 해저드 존, 피크의 OVER
  태그)를 읽습니다. 그래프에 포인터 판독도 생겼습니다. 마우스를 올리면
  커서 아래의 정확한 주파수와 dB를 각 스킨의 어법으로 읽어줍니다.
  ([#172](https://github.com/115dkk/EqualizerAPO-XT/pull/172))
