...
- **VST3 플러그인의 주 입출력 버스를 서로 다르게 명시할 수 있습니다.** 새
  백엔드 명령 `VST3Bus:`는 입력과 출력에 `Auto`, Mono, Stereo, 4.0, 4.1,
  5.0, 5.1, 6.1, 7.1, 7.1.2, 7.1.4를 각각 지정합니다. 명시한 레이아웃을
  플러그인이 거부하거나 다른 배열·폭을 보고하면 다른 레이아웃으로 후퇴하지 않고 모든
  장치 채널을 그대로 통과시킵니다. VST2는 확장자가 아니라 실제 로드된 ABI로
  판별해 거부합니다. Editor 컨트롤은 별도 작업으로 추가할 예정입니다.
