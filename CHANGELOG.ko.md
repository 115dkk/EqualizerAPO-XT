...
- 클린 설치 직후 가장 먼저 보이는 그래픽 EQ가 모던 카드 에디터를 얻었습니다.
  그래프는 20 Hz–20 kHz 전체가 항상 카드 폭에 맞고(빈 +100 dB 구석으로
  가로 스크롤되던 문제 소멸), 응답을 중심으로 프레이밍되며, 색은 전부 현재
  스킨에서 나옵니다. 밴드 표와 버튼은 스킨마다 자기 문법으로 그려집니다
  (Studio의 침강 유리 데이터 창, Minimal의 줄무늬 문서, Soft의 둥근 우물,
  Rack의 녹색 세그먼트 LCD, Matrix의 보드 데이터 셀). 레거시 행 모드의
  원본 GUI는 그대로입니다.
