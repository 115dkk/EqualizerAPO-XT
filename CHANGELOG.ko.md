...
- **Soft 스킨을 종이와 잉크로 다시 입혔습니다.** 앞선 라운드의 파스텔
  선반(파랑·앰버·초록·보라 타일, 초록 저장 알약, 채널 배지 8색, 분석 곡선
  아래의 지형 채움)을 걷어냈습니다. 이제 이 스킨의 색은 둘뿐입니다. 고르거나
  쥔 것에는 청회색 액센트 하나(스위치는 액센트로 채우고, 선택된 알약은 옅은
  틴트 위에 액센트 잉크), 살펴볼 것에는 앰버 잉크 하나(없는 파일, 저장 안
  된 변경, 0 dB를 넘은 응답)입니다. 카드는 밝은 트레이 대신 2px 어두운 받침
  위의 웜 페이퍼 한 장이고, 타입 표시는 잉크 픽토그램만 남으며, 노브는 얇은
  호와 받침 위의 종이 면이고, 값 상자는 테두리 없는 우물, 분석 그래프는 패널
  위의 액센트 선 하나, 툴바는 글리프 옆에 동작 이름을 붙입니다. 배치는
  그대로여서 어떤 컨트롤도 자리를 옮기지 않았습니다. 화면 컨셉은
  [#341](https://github.com/115dkk/EqualizerAPO-XT/issues/341)에서 메인테이너가
  채택한 Soft 목업을 따르고, 그 목업의 배치는 채택하지 않았습니다.
