...
- rack 스킨의 Copy 라우팅은 하드웨어 라우팅 매트릭스의 버튼 필드가
  ...
  그대로, 음수 게인은 danger 램프), 유니티 라우팅은 legend 없이 램프
  창만 켜서 패널의 어떤 값도 음소거로 오독되지 않습니다. 필터 카드의
  로터리 다이얼과는 일부러 다른 부품입니다. 팩터가 없는
  MultiConvolution의 패치 포인트도 같은 램프 창을 씁니다. ([#141])
