...
7. Explicit VST3 bus layouts ([#216](https://github.com/115dkk/EqualizerAPO-XT/issues/216)) — the backend `VST3Bus:` command and deterministic host tests are complete, including asymmetric input/output, 4.1, strict failure passthrough, and VST2 rejection. Qt Editor selectors and cards are a separate follow-up; for now the command is written by hand.
...
- Explicit asymmetric VST3 main buses through `VST3Bus: Library "...\\Plugin.vst3" Input Stereo Output 7.1`. Input and output independently support Auto, Mono, Stereo, 4.0, 4.1, 5.0, 5.1, 6.1, 7.1, 7.1.2, and 7.1.4; rejected contracts safely pass audio through instead of silently choosing another arrangement or width. See the [configuration reference](https://github.com/115dkk/EqualizerAPO-XT/wiki/Configuration-reference#vst3bus).
