...
6. 베이스 매니지먼트([#246](https://github.com/115dkk/EqualizerAPO-XT/issues/246)) — 핵심 기능은 완료됐습니다. `BassManagement:` 명령, MIT BassManagementCore DSP 라이브러리, 독립 실행형 VST3 플러그인, 4.1 호스트 협상 수정, 다섯 스킨 각각의 카드 계기, 그리고 두 라우팅 행렬과 응답 뷰를 갖춘 전체 편집기까지 들어갔습니다. 남은 후속 작업은 연결된 프로필 파일로의 변경 사항 되쓰기(현재는 행을 인라인 상태로 전환), audition/solo 오버라이드, 새 문자열의 한국어 번역, 전용 VST3 편집기 화면(현재는 호스트의 일반 파라미터 화면)입니다.
...
- 희소 벨벳 노이즈 비상관화 필터가 내장되어 있습니다.
  ...
  별도로 배포합니다.
- 한 줄 베이스 매니지먼트: `BassManagement:`가 스피커 그룹별 크로스오버, 전용 베이스 경로, 물리 LFE 입력 보존, 경로별 게인·극성·지연·EQ, 출력 합산 행렬을 JSON 상태 하나로(인라인 또는 `*.bmxt.json` 프로필) 실행합니다. 자동 헤드룸이 합산 출력을 지키고, 내장 프리셋은 이슈 #246의 원본 사슬을 샘플 단위로 재현합니다. 같은 MIT 라이선스 DSP 코어가 독립 실행형 `EAPO XT Bass Management` VST3 플러그인으로도 실려, 동일한 JSON 상태를 주고받습니다.
