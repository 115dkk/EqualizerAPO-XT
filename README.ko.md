...
4. 프로그래밍 계열 설정 명령(`If:`/`ElseIf:`/`Else:`/`EndIf:`/`Eval:`) 전용 에디터를 만듭니다. 카드 모델이 명령을 인식하고([#178](https://github.com/115dkk/EqualizerAPO-XT/pull/178)) 다섯 스킨이 분석 실행이 알려 주는 판정으로 블록을 각자의 계기로 표현합니다([#182](https://github.com/115dkk/EqualizerAPO-XT/pull/182)). 남은 작업은 스킨 헌법 절, 픽토그램, 픽커 템플릿, 번역입니다.

## 주요 기능

- 오디오를 내부에서 double 정밀도로 처리해 복잡한 필터 체인에서도 정밀도를 잃지 않습니다.
- Convolution, GraphicEQ, 파라메트릭 EQ, VST2/VST3와 기존 Equalizer APO 필터를 지원합니다.
- 트루 스테레오와 BRIR(Binaural Room Impulse Response) 재생을 위한 MultiConvolution 필터가 있습니다. `MultiConvolution: L=0+1 R=2+3 brir.wav`처럼 각 채널 자신의 신호를 매핑된 파일 채널들과 컨볼루션해 합산하며, Channel 명령과 무관하게 동작합니다. 제자리 처리만 하는 기존 Convolution 필터가 표현하지 못하는 분기·합산 패턴이 한 줄로 끝나고, Editor에서는 스킨마다 다른 라우팅 화면으로 매핑을 편집합니다.
