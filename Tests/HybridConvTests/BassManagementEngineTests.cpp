...
test::Harness harness("BassManagementEngineTests");
...
{
	const std::wstring deviceName = L"BassManagementEngineTests";
...
private:
	...
	void createFiles()
	{
		bassmgmt::PresetCreateResult preset =
		...
		bassmgmt::BassManagementState state = *preset.state;
		state.headroom.mode = bassmgmt::HeadroomMode::Manual;
		state.headroom.manualTrimDb = 0.0;

		bassmgmt::StateEncodeResult encoded =
...
void testInvalidStateDoesNotMute(const SuiteFixture& fixture)
{
	...
	harness.expectTrue(
		loaded,
		"an invalid BassManagement state line must not fail the overall engine config load");
	...
	harness.expectTrue(
		identical,
		"an invalid BassManagement state must leave audio bit-exactly unchanged");
}
