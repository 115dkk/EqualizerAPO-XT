...
test::Harness harness("BassManagementJsonTests");
...
void testLiteralsNestingAndEscapes()
{
	...
	const auto result = bassmgmt::parseJson(
	...
	const bassmgmt::Json& array = result.value->at("array");
	...
	const bassmgmt::Json& text = array.at(3).at("text");
...
void testInvalidUtf8Rejected()
{
	...
	const auto result = bassmgmt::parseJson(text);
	...
	harness.expectTrue(
		result.error->code == bassmgmt::JsonParseErrorCode::InvalidUtf8,
...
{
	const auto result = bassmgmt::parseJson(R"({"a":1,"a":2})");
	...
	harness.expectTrue(
		result.error->code == bassmgmt::JsonParseErrorCode::DuplicateObjectKey,
...
{
	const auto result = bassmgmt::parseJson("null false");
	...
	harness.expectTrue(
		result.error->code == bassmgmt::JsonParseErrorCode::TrailingContent,
...
{
	const auto result = bassmgmt::parseJson("[0,]");
	...
	harness.expectTrue(
		result.error->code == bassmgmt::JsonParseErrorCode::TrailingComma,
...
void testDepthLimit()
{
	bassmgmt::JsonParseOptions options;
	options.maximumDepth = 2;

	const auto accepted = bassmgmt::parseJson("[[0]]", options);
	harness.expectTrue(
		accepted.succeeded(),
		"document at the configured container depth should parse");

	const auto rejected = bassmgmt::parseJson("[[[0]]]", options);
	harness.expectFalse(
		rejected.succeeded(),
	...
	harness.expectTrue(
		rejected.error->code == bassmgmt::JsonParseErrorCode::MaximumDepthExceeded,
...
void testDoubleRoundTrips()
{
	...
	{
		const auto serialized = bassmgmt::serializeJson(bassmgmt::Json(expected));
		harness.require(
			serialized.succeeded(),
			"finite double should serialize");

		const auto parsed = bassmgmt::parseJson(*serialized.text);
		harness.require(
			parsed.succeeded(),
...
{
	bassmgmt::Json::Object object;
	object.emplace("z", bassmgmt::Json(1.0));
	object.emplace("a", bassmgmt::Json(2.0));
	object.emplace("m", bassmgmt::Json(3.0));

	const auto result =
		bassmgmt::serializeJson(bassmgmt::Json(std::move(object)));
