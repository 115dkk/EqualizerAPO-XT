...
#include "VST3/BassManagement/plugin_ids.h"
...
test::Harness harness("BassManagementVst3Tests");
...
void runBassManagementVst3Tests()
{
	...
	using namespace eapoxt::bassmanagement::vst3;
	...
	const tresult created = library->getFactory()->createInstance(
		eapoxt::bassmanagement::vst3::kComponentCid,
		componentIid,
	...
	BassManagementState desired = *preset.state;
	...
	harness.expectTrue(maximumDifference <= 1.0e-12,
		"VST3 double output matches BassManagementCore within 1e-12");
