/*
	...
	The feeder itself is Qt code compiled into the Editor; the loopback
	premise is re-implemented standalone here so the probe runs headless.
	Exit codes: 0 ok, 1 usage, 2 load/negotiation failure, 3 processing
	failure, 4 loopback failure.
*/
...
int wmain(int argc, wchar_t* argv[])
{
	...
	for (int i = 1; i < argc; i++)
	{
		...
		{
			wprintf(L"Usage: VstPreviewProbe [--plugin <path.vst3|path.dll>] [--seconds N] [--rate N]\n"
				L"                       [--expect-nonsilent] [--loopback]\n");
			return 1;
		}
	}
	if (pluginPath.empty() && !loopback)
	{
		wprintf(L"Usage: VstPreviewProbe [--plugin <path.vst3|path.dll>] [--seconds N] [--rate N]\n"
			L"                       [--expect-nonsilent] [--loopback]\n");
		return 1;
