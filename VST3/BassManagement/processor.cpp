...
struct BassManagementProcessor::PreparedEngine
{
...
BassManagementProcessor::BassManagementProcessor()
	: arrangement_(SpeakerArr::k41Music)
{
	...
	state_ = *preset.state;
	const bassmgmt::StateEncodeResult encoded = bassmgmt::encodeStateCanonical(state_);
...
tresult PLUGIN_API BassManagementProcessor::queryInterface(const TUID iid, void** object)
{
	if (object == nullptr)
...
uint32 PLUGIN_API BassManagementProcessor::addRef()
{
	return ++refCount_;
}

uint32 PLUGIN_API BassManagementProcessor::release()
{
...
tresult PLUGIN_API BassManagementProcessor::initialize(FUnknown* context)
{
...
tresult PLUGIN_API BassManagementProcessor::terminate()
{
...
tresult PLUGIN_API BassManagementProcessor::getControllerClassId(TUID classId)
{
	if (classId == nullptr)
		return kInvalidArgument;
...
tresult PLUGIN_API BassManagementProcessor::setIoMode(IoMode)
{
	return kResultOk;
}

int32 PLUGIN_API BassManagementProcessor::getBusCount(MediaType type, BusDirection)
{
	return type == kAudio ? 1 : 0;
}

tresult PLUGIN_API BassManagementProcessor::getBusInfo(
	MediaType type,
...
tresult PLUGIN_API BassManagementProcessor::getRoutingInfo(RoutingInfo&, RoutingInfo&)
{
	return kNotImplemented;
}

tresult PLUGIN_API BassManagementProcessor::activateBus(
	MediaType type,
	BusDirection,
	int32 index,
	TBool)
...
tresult PLUGIN_API BassManagementProcessor::setActive(TBool state)
{
	std::lock_guard<std::mutex> lock(stateMutex_);
...
bool BassManagementProcessor::readFramedState(IBStream* stream, std::string& json)
{
	if (stream == nullptr)
		return false;
...
tresult BassManagementProcessor::writeFramedState(IBStream* stream, const std::string& json)
{
	if (stream == nullptr)
		return kInvalidArgument;
...
tresult PLUGIN_API BassManagementProcessor::setState(IBStream* stream)
{
	std::string incomingJson;
	if (!readFramedState(stream, incomingJson))
		return kResultFalse;

	const bassmgmt::StateDecodeResult decoded = bassmgmt::decodeState(incomingJson);
...
tresult PLUGIN_API BassManagementProcessor::getState(IBStream* stream)
{
	std::lock_guard<std::mutex> lock(stateMutex_);
	return writeFramedState(stream, canonicalJson_);
}

bool BassManagementProcessor::isAcceptedArrangement(SpeakerArrangement arrangement)
{
...
std::vector<std::string> BassManagementProcessor::channelLayoutForArrangement(
...
tresult PLUGIN_API BassManagementProcessor::setBusArrangements(
...
tresult PLUGIN_API BassManagementProcessor::getBusArrangement(
	BusDirection,
	int32 index,
	SpeakerArrangement& arrangement)
{
...
tresult PLUGIN_API BassManagementProcessor::canProcessSampleSize(int32 symbolicSampleSize)
{
...
uint32 PLUGIN_API BassManagementProcessor::getLatencySamples()
{
	return 0;
}

tresult PLUGIN_API BassManagementProcessor::setupProcessing(ProcessSetup& setup)
{
	if (active_.load(std::memory_order_acquire)
...
tresult PLUGIN_API BassManagementProcessor::setProcessing(TBool state)
{
...
void BassManagementProcessor::consumeParameterChanges(IParameterChanges* changes) noexcept
{
	if (changes == nullptr)
		return;
...
tresult PLUGIN_API BassManagementProcessor::process(ProcessData& data)
{
...
uint32 PLUGIN_API BassManagementProcessor::getTailSamples()
{
	...
	// host tail for the supported bass-management crossover ranges and avoids
...
tresult PLUGIN_API BassManagementProcessor::connect(IConnectionPoint* other)
{
	if (other == nullptr)
		return kInvalidArgument;
...
tresult PLUGIN_API BassManagementProcessor::disconnect(IConnectionPoint* other)
{
	std::lock_guard<std::mutex> lock(stateMutex_);
	if (peer_ == nullptr)
		return kResultOk;
...
tresult PLUGIN_API BassManagementProcessor::notify(IMessage* message)
{
	if (message == nullptr
...
std::unique_ptr<BassManagementProcessor::PreparedEngine>
BassManagementProcessor::buildPrepared(
	...
	const ProcessSetup& setup) const
{
	...
	bassmgmt::PrepareSpec specification;
	...
	bassmgmt::CompileResult compiled = bassmgmt::compile(state, specification);
...
void BassManagementProcessor::publish(std::unique_ptr<PreparedEngine> replacement)
{
	reapRetired();
...
void BassManagementProcessor::reapRetired()
{
...
void BassManagementProcessor::clearPublishedEngineLocked()
{
	publish(nullptr);
}

bool BassManagementProcessor::applyPendingParametersLocked()
{
	...
	const bassmgmt::StateEncodeResult encoded =
		bassmgmt::encodeStateCanonical(state_);
	if (!encoded.succeeded())
		return false;
	canonicalJson_ = *encoded.text;
...
bool BassManagementProcessor::applyParameterLocked(
	ParamID id,
	...
	bool rebuild)
{
	...
	bassmgmt::BassManagementState candidate = state_;
	bassmgmt::Path* sourceLfe = findSourceLfePath(candidate);
	...
	else if (id == kSourceLfePolarityParamId)
	{
		if (sourceLfe == nullptr)
			return false;
		bool found = false;
		for (bassmgmt::PathStage& stage : sourceLfe->chain)
		{
			if (bassmgmt::PolarityStage* polarity =
	...
	else if (id == kOutputTrimParamId)
	{
		...
		if (std::fabs(requested - automatic) > 1.0e-9)
			candidate.headroom.mode = bassmgmt::HeadroomMode::Manual;
	}
	...
	const bassmgmt::StateEncodeResult encoded =
		bassmgmt::encodeStateCanonical(candidate);
	if (!encoded.succeeded())
		return false;
...
bool BassManagementProcessor::rebuildCurrentLocked()
{
	std::unique_ptr<PreparedEngine> replacement;
	try
	{
...
FUnknown* createBassManagementProcessor()
{
	return static_cast<IComponent*>(new BassManagementProcessor());
}
