...
const bassmgmt::Path* findSourceLfePath(const bassmgmt::BassManagementState& state)
{
...
BassManagementController::BassManagementController()
{
...
tresult PLUGIN_API BassManagementController::queryInterface(const TUID iid, void** object)
{
	if (object == nullptr)
...
uint32 PLUGIN_API BassManagementController::addRef()
{
	return ++refCount_;
}

uint32 PLUGIN_API BassManagementController::release()
{
...
tresult PLUGIN_API BassManagementController::initialize(FUnknown* context)
{
...
tresult PLUGIN_API BassManagementController::terminate()
{
...
bool BassManagementController::readFramedState(IBStream* stream, std::string& json)
{
	if (stream == nullptr)
		return false;
...
tresult PLUGIN_API BassManagementController::setComponentState(IBStream* stream)
{
	std::string json;
	if (!readFramedState(stream, json))
		return kResultFalse;

	const bassmgmt::StateDecodeResult decoded = bassmgmt::decodeState(json);
...
tresult PLUGIN_API BassManagementController::setState(IBStream*)
{
	return kNotImplemented;
}

tresult PLUGIN_API BassManagementController::getState(IBStream*)
{
	return kNotImplemented;
}

int32 PLUGIN_API BassManagementController::getParameterCount()
{
	return 6;
}

tresult PLUGIN_API BassManagementController::getParameterInfo(int32 index, ParameterInfo& info)
{
...
tresult PLUGIN_API BassManagementController::getParamStringByValue(
	ParamID id,
	ParamValue value,
	String128 string)
{
...
tresult PLUGIN_API BassManagementController::getParamValueByString(
	ParamID id,
	TChar* string,
	ParamValue& value)
{
...
ParamValue PLUGIN_API BassManagementController::normalizedParamToPlain(
	ParamID id,
	ParamValue value)
{
...
ParamValue PLUGIN_API BassManagementController::plainParamToNormalized(
	ParamID id,
	ParamValue value)
{
	switch (id)
...
ParamValue PLUGIN_API BassManagementController::getParamNormalized(ParamID id)
{
...
tresult PLUGIN_API BassManagementController::setParamNormalized(
	ParamID id,
	ParamValue value)
{
	const int index = parameterIndex(id);
...
tresult PLUGIN_API BassManagementController::setComponentHandler(
...
IPlugView* PLUGIN_API BassManagementController::createView(FIDString)
{
	return nullptr;
}

tresult PLUGIN_API BassManagementController::connect(IConnectionPoint* other)
{
	if (other == nullptr)
		return kInvalidArgument;
...
tresult PLUGIN_API BassManagementController::disconnect(IConnectionPoint* other)
{
	std::lock_guard<std::mutex> lock(mutex_);
	if (peer_ == nullptr)
		return kResultOk;
...
tresult PLUGIN_API BassManagementController::notify(IMessage*)
{
	return kResultOk;
}

void BassManagementController::updateValuesFromState(
	const bassmgmt::BassManagementState& state)
{
	values_[0] = 0.0;

	const bassmgmt::Path* sourceLfe = findSourceLfePath(state);
	if (sourceLfe != nullptr)
	{
		...
		for (const bassmgmt::PathStage& stage : sourceLfe->chain)
		{
			if (const bassmgmt::PolarityStage* polarity =
				std::get_if<bassmgmt::PolarityStage>(&stage))
			{
				values_[2] = polarity->inverted ? 1.0 : 0.0;
			}
			else if (const bassmgmt::DelayStage* delay =
	...
	const bassmgmt::CompileResult compiled = bassmgmt::compile(state, specification);
	...
	const double displayedTrim = state.headroom.mode == bassmgmt::HeadroomMode::Auto
		? automaticTrimDb_
		: state.headroom.manualTrimDb;
	values_[4] = toNormalized(displayedTrim, -40.0, 0.0);
	values_[5] = state.headroom.mode == bassmgmt::HeadroomMode::Auto ? 1.0 : 0.0;
}

bool BassManagementController::sendParameter(ParamID id, ParamValue value)
{
	IHostApplication* host = nullptr;
...
int BassManagementController::parameterIndex(ParamID id) const
{
...
FUnknown* createBassManagementController()
{
	return static_cast<IEditController*>(new BassManagementController());
}
