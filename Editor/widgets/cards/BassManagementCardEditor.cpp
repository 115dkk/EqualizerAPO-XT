#include "BassManagementCardEditor.h"
...
bassmgmt::PathStage polarityStage()
{
	return bassmgmt::PolarityStage{false};
}

bassmgmt::PathStage delayStage()
{
	return bassmgmt::DelayStage{0.0};
}

bassmgmt::PathStage equalizerSlotsStage()
{
	return bassmgmt::EqualizerSlotsStage{};
}

bassmgmt::Path makePath(const std::string& id,
	bassmgmt::PathKind kind,
	const std::vector<bassmgmt::SourceMixTerm>& sourceMix,
	std::optional<bassmgmt::BiquadType> crossoverType)
{
	bassmgmt::Path path;
	path.id = id;
	path.kind = kind;
	...
	if (crossoverType.has_value())
	{
		...
		filter.gainDb = 0.0;
		path.chain.push_back(bassmgmt::BiquadStage{filter});
	}
	...
	const std::vector<std::wstring>& channels)
{
	...
	{
		const std::string id = bassManagementToUtf8(channel);
		if (!id.empty() && bassmgmt::isValidStableId(id)
...
QString firstDiagnostic(
	...
	bassmgmt::DiagnosticSeverity severity)
{
...
QString layoutLabel(const bassmgmt::BassManagementState& state)
{
	int lfeChannels = 0;
...
double sourceLfeGainDb(const bassmgmt::Path& path)
{
	...
	for (const bassmgmt::PathStage& stage : path.chain)
	{
		const bassmgmt::GainStage* gain =
			std::get_if<bassmgmt::GainStage>(&stage);
		if (gain != nullptr)
			gainDb += gain->gainDb;
	}
	return gainDb;
...
double compiledTrimDb(const bassmgmt::HeadroomAnalysis& analysis)
{
	return analysis.appliedTrimDb;
}
QString presetDisplayName(
	const bassmgmt::PresetDescriptor& preset)
{
	if (preset.id == bassmgmt::kIssue246FrontRear41PresetId)
		return BassManagementCardEditor::tr(
	...
	const std::vector<std::wstring>& deviceChannels)
{
	...
	state.headroom.mode = bassmgmt::HeadroomMode::Auto;
	state.headroom.manualTrimDb = 0.0;

	const std::optional<bassmgmt::BiquadType> mainCrossover =
		hasLfe
		? std::optional<bassmgmt::BiquadType>(
			bassmgmt::BiquadType::HighPass)
		: std::nullopt;

	state.paths.push_back(makePath("FrontLeft",
		bassmgmt::PathKind::Main, {{left, 1.0}},
		mainCrossover));
	state.paths.push_back(makePath("FrontRight",
		bassmgmt::PathKind::Main, {{right, 1.0}},
		mainCrossover));
	...
	state.outputMatrix.push_back(
		{left, bassmgmt::OutputMode::Replace,
			{{"FrontLeft", 0.0}}});
	state.outputMatrix.push_back(
		{right, bassmgmt::OutputMode::Replace,
			{{"FrontRight", 0.0}}});

	if (hasLfe)
	{
		state.outputMatrix.push_back(
			{lfeId, bassmgmt::OutputMode::Replace,
...
BassManagementCardEditor::BassManagementCardEditor(
	FilterTable* table, const BassManagementCommand& command,
	const QString& path, unsigned sampleRate, QWidget* parent)
	: BassManagementCardEditor(table, command, path, sampleRate,
...
{
	setObjectName(QStringLiteral("BassManagementCardEditor"));
	...
	view = SkinManager::instance()->createBassManagementCardView(this);
	layout->addWidget(view);
	connect(view, &BassManagementCardView::openEditorRequested,
	...
	openButton->setIcon(GUIHelper::tintedIcon(
		QStringLiteral(":/icons/modern/bass-management.svg"),
		actionColor, 18));
	...
	presetButton->setToolTip(tr("Choose a built-in bass-management preset"));
	...
	rowInfo.type = QStringLiteral("bassmanagement");
	rowInfo.command = QStringLiteral("bassmanagement");
...
void BassManagementCardEditor::loadCommand(
	const BassManagementCommand& command, const QString& parseError)
{
	form = command.form;
	...
	if (command.form == BassManagementCommand::Form::State)
	{
		...
		const std::string payload =
			bassManagementToUtf8(command.payload);
	...
	const bassmgmt::StateDecodeResult decoded =
		bassmgmt::decodeState(std::string_view(
	...
	QString& command, QString& parameters)
{
	...
	const bassmgmt::StateEncodeResult encoded =
		bassmgmt::encodeStateCanonical(*currentState);
...
void BassManagementCardEditor::configureChannels(
	...
	const std::string& presetId)
{
	const bassmgmt::PresetCreateResult preset =
		bassmgmt::createBuiltInPreset(presetId);
	if (!preset.succeeded())
	{
...
QString BassManagementCardEditor::resolvedProfilePath(
...
void BassManagementCardEditor::refreshCard()
{
	BassManagementCardState card;
	card.enabled = isEnabled();
	card.linkedProfile =
		form == BassManagementCommand::Form::Profile;
	...
	const bassmgmt::BassManagementState& state = *currentState;
	const bassmgmt::ValidationResult validation =
	...
	for (const bassmgmt::Path& path : state.paths)
	{
		...
		for (const bassmgmt::OutputMatrixEntry& output
			: state.outputMatrix)
		{
			for (const bassmgmt::OutputMatrixTerm& term
				: output.terms)
			{
...
REGISTER_FILTER_CARD_EDITOR(BassManagement,
		...
		const QString& parameters) -> IFilterGUI*
	{
		...
		{
			std::wstring error;
			if (!BassManagementCommand::parse(
		...
		return new BassManagementCardEditor(table, parsed,
