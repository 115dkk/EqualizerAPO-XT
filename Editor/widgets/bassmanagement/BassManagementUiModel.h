...
class BassManagementUiModel : public QObject
{
	Q_OBJECT

public:
	explicit BassManagementUiModel(
		const bassmgmt::BassManagementState& state,
		unsigned deviceSampleRate,
		QObject* parent = nullptr);

	const bassmgmt::BassManagementState& state() const;
	const bassmgmt::ValidationResult& validation() const;
	...
	void applyOutputAssignments(
		const std::vector<Assignment>& assignments);
	void replaceState(const bassmgmt::BassManagementState& state);
...
private:
	...
	bassmgmt::BassManagementState currentState;
	bassmgmt::ValidationResult currentValidation;
